...
function [u_final, num_iterations, error] = Conjugate_Residual(epsilon, ...
    initial_guess, b, problem, p, num_grid_points)
    % This function is going to implement the conjugate residual method to
    ...
    %   converge
    %   error = final error of the CR solver
    ...
    % REDACTED: CR code is available on request

    % Update the return value with the result from the conjugate residual
    % solver
    u_final = x_new;
end